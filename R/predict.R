...
#' @export
#'
predict_evgam_egpd <- function(object, newdata, type="link", prob=NULL, se.fit=FALSE, marginal=TRUE, 
exi = FALSE, trace = 0, ...) {
...
if (type == "qqplot" & !(family %in% c("gev", "gpd", "weibull", "egpd")))
...
if (family == "egpd") {
  out[,4]  = (out[,2]/(1+out[,3]))
  names(out)[c(2,4)] = c("mod_scale", "scale")
}
