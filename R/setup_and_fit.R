...
.setup.data <- function(data, responsename, formula, family, nms, removeData, 
  ...
  if (attr(formula, "censored")) {
    ...
    #lik.data$y[,2] <- pmax(lik.data$y[,2], lik.data$y[,1])
  ...
  if (family == "egpd") {
    if (min(lik.data$y) <= 0) 
      stop(expression("egpd distribution has support (0, \U221E) in evgam."))
  }
...
.setup.inner.inits <- function(inits, likdata, likfns, npar, family) {
  ...
  if (is.null(inits)) {
    ...
    if (npar %in% 3) {
      ...
      if (likdata0$censored) {
        inits <- fevd(likdata0$y[,2], method = "MLE", type="GP", threshold=0, 
                      optim.args=list(method="L-BFGS-B", lower =c(1, 0.001), upper = c(100, 0.3)))$results$par
      } else{
        inits <- fevd(likdata0$y[,1], method = "MLE", type="GP", threshold=0, 
                      optim.args=list(method="L-BFGS-B", lower =c(1, 0.001), upper = c(100, 0.3)))$results$par
      }
      #inits <- c(gp.fit(likdata0$y[,1], 0)$est) # negative xi produced sometimes, resulting in NANS for log
      #recall we use v instead of sigma here
      inits <- c(log(0.9), log((inits[1])*(1+inits[2])), log(inits[2]))
    }
    likdata0$CH <- diag(length(inits))
    likdata0$compmode <- numeric(length(inits))
    beta0 <- .newton_step_inner(pars = inits, fn = .nllh.nopen, sfn =  .search.nopen, likdata=likdata0, likfns=likfns, control=likdata$control$inner)$par
  } else {
    if (is.list(inits)) {
...
.finalise <- function(gams, data, likfns, likdata, Sdata, fitreml, VpVc, family, gotsmooth,
  ...
  nms <- c("logkappa", "logscale", "logshape")
  names(gams) <- nms
